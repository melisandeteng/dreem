...
def apply_model(small_dataset, saving_results, storing_small_dataset, output_name, convolution, param):
    ...
    # model
    if convolution:
        train_pred, valid_pred, test_pred, train_score, valid_score = convolution_model(
            train_data_eeg, train_data_resp, train_data_meta, train_data_y,
            valid_data_eeg, valid_data_resp, valid_data_meta,
            test_data_eeg, test_data_resp, test_data_meta, valid_data_y, nb_epoch
        )
    else:
        results = [[]]
        for p in param:
            train_pred, valid_pred, test_pred, train_score, valid_score \
                = gradient_boost_model(train_data_x, train_data_y, valid_data_x, valid_data_y, test_data_x, p)
            results += [[p, train_score, valid_score]]
        print(results)
...
def convolution_model(train_data_eeg, train_data_resp, train_data_meta, train_data_y,
                      valid_data_eeg, valid_data_resp, valid_data_meta,
                      test_data_eeg, test_data_resp, test_data_meta, valid_data_y, nb_epoch):
    # define model
    ...
    model.fit([train_data_eeg, train_data_resp, train_data_meta], train_data_y,
              epochs=nb_epoch, validation_data=([valid_data_eeg, valid_data_resp, valid_data_meta], valid_data_y),
              batch_size=25)
...
def gradient_boost_model(train_data_x, train_data_y, valid_data_x, valid_data_y, test_data_x, parameter):
    ...
    train_score = math.sqrt(mean_squared_error(train_data_y, train_pred))
    valid_score = math.sqrt(mean_squared_error(valid_data_y, valid_pred))
    print('Param: %s \nTrain Score: %.2f RMSE \nValidation Score: %.2f RMSE' % (parameter, train_score, valid_score))
